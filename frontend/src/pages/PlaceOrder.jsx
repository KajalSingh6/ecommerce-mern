  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      switch (method) {
        // API Calls for COD
        case 'cod':
          const res = await axios.post(`${backendUrl}/api/order/place`, orderData, { headers: { Authorization: token } });
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row '>
            ...
            <div onClick={() => setMethod('cod')} className='flex items-center gap-3 border cursor-pointer px-3 p-2 h-15 w-full'>
              <p className={`min-w-3.5 h-3.5 rounded-full ${method === 'cod' ? 'bg-green-400 ' : ''}`}></p>
              <p className='text-gray-500 text-sm font-medium mx-4'>CASH ON DELIVERY</p>
            </div>
          </div>
          <div className='w-full text-end mt-8'>
            <button type='submit' className='text-sm py-3 px-16 bg-black text-white cursor-pointer w-full'>PLACE ORDER</button>
